...
(function () {
	...
	QRPolynomial.prototype = {
		...
		multiply: function (e) {
			...
			for (var i = 0; i < this.getLength(); i++) {
				for (var j = 0; j < e.getLength(); j++) {
					num[i + j] ^= QRMath.gfmul(this.get(i), e.get(j));
				}
			}
		...
		mod: function (e) {
			...
			for (var i = 0; i < e.getLength(); i++) {
				num[i] ^= QRMath.gfmul(e.get(i), QRMath.gexp(ratio));
			}
			return (new QRPolynomial(num, 0)).mod(e);
		}
	};
	QRMath.gfmul = function (a, b) {
		if (a == 0 || b == 0) {
			return 0;
		}
	...
	QRCode.prototype.renderToCanvas = function (canvas) {
		if (!this._oQRCode) {
		...
		var nRoundedWidth = Math.round(nWidth / nCount);
		var nRoundedHeight = Math.round(nHeight / nCount);
